...
const fetchProductData = async (productName) => {
  try {
    const response = await fetch(
      `http://127.0.0.1:8000/product/${encodeURIComponent(productName)}`
    );
...
const fetchStockTrend = async (location, productName) => {
  try {
    const response = await fetch(
      `http://127.0.0.1:8000/stock_trend?location=${encodeURIComponent(
        location
...
export default function NextPage() {
  ...
  const optimalPrice =
    discountData?.optimal_price ||
    (formData.MRP ? (formData.MRP * 0.9).toFixed(2) : 0);

  // Get product image URL - properly falling back to unsplash if needed
  const getProductImageUrl = () => {
    ...
    // If no product image from API, use a dynamic unsplash image based on product name
              ...
              {isLoading ? (
                ...
                <img
                  ...
                  onError={(e) => {
                    ...
                    e.target.parentNode.innerHTML += '<p>Unable to load trend data</p>';
                  }}
                />
              ) : (
            ...
            <>
              ...
              <p style={styles.productDesc}>
                {productData.description ||
                  `This is a premium quality product available in ${
